...
const ll BASE[] = {1572872831LL, 1971536491LL};
...
int main()
{
    ...
    while (cin >> x >> needle >> haystack)
    {
        ...
        Construct(needle);
        ll needleHash0 = GetForwardHash0(1, needle.size());
        ll needleHash1 = GetForwardHash1(1, needle.size());

        Construct(haystack);

        int i = 1, j = x;
        while (i <= haystack.size() and j <= haystack.size())
        {
            int ii = i, jj = i + x - 1;
            if (GetForwardHash0(ii, jj) == needleHash0 and GetForwardHash1(ii, jj) == needleHash1)
            {
                cout << i - 1 << nl;
            }
            i++;
