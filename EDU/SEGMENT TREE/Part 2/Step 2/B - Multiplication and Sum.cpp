...
struct ST {
    ...
    void push(int node, int lx, int rx)
    {
        ...
        int mid = (lx + rx) >> 1;
        lazy[node * 2] = _modify(lazy[node * 2],lazy[node]);
        lazy[node * 2 + 1 ] = _modify(lazy[node * 2+1],lazy[node]);
        Tree[node*2]= _modify(Tree[node*2],lazy[node]);
        Tree[node*2+1]= _modify(Tree[node*2+1],lazy[node]);
        lazy[node] = 0LL;
    }
    ...
    ll query(int node, int lx, int rx, int l, int r)
    {
        ...
        if (lx >= l and r >= rx)
        {
            return Tree[node]%mod; //রিলেভেন্ট সেগমেন্ট
        }
        ...
        return (leftQ + rightQ)%mod; //বাম এবং ডান পাশের যোগফল
    }
    ...
    void update(int node, int lx, int rx, int l, int r, ll val)
    {
        ...
        {
            //  cout<<lx<<" "<<rx<<nl;
            Tree[node]= _modify(Tree[node],val);
            lazy[node]=_modify(lazy[node],val);
            return;
        }
